 ...
 *
 * `error` = retrying (transient — 5xx / network). `error-terminal` = 4xx,
 * no recovery without user action.
 */
  ...
  return (
    <div role="status" aria-live="polite" aria-atomic="true" className="text-sm" data-testid="auto-save-indicator">
      ...
      {state === 'error-terminal' && (
        <span className="text-danger" data-testid="auto-save-terminal-error">
          Save failed — please reload
        </span>
      )}
    </div>
  )
